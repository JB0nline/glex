...
GameAssetManager::GameAssetManager(ApplicationMode mode) {
	...
	projectionMatrix_link = glGetUniformLocation(program_token,
			"projectionMatrix");
	translateMatrix_link = glGetUniformLocation(program_token,
			"translateMatrix");
...
void GameAssetManager::UpdateCameraPosition(Input input_Direction, int mouseX,
		int mouseY) {
